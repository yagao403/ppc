        ...
        {% for k in ['inner', 'outer'] %}
            {% for i in [0, 1, 2] %}
                {% if too_large(output, expected, k, i) %}
                    <tr>
                        <td>{{k}}[{{i}}]</td>
                        <td class="right"><strong>{{safeprint(safeget(output, k, i))}}</strong></td>
                        <td class="right"><strong>{{safeprint(safeget(expected, k, i))}}</strong></td>
                    </tr>
                {% else %}
                    <tr>
                        <td>{{k}}[{{i}}]</td>
                        <td class="right">{{safeprint(safeget(output, k, i))}}</td>
                        <td class="right">{{safeprint(safeget(expected, k, i))}}</td>
                    </tr>
                {% endif %}
            {% endfor %}
        {% endfor %}
...
"""
    ...
    return render_explain_web(templ_basic,
                              raw,
                              expected=raw.get("output_errors",
                                               {}).get("expected", {}),
                              too_large=too_large)
...
def explain_terminal(r, color=False):
    ...
    if all(x in input for x in par):
        ...
        if 'data' in input:
            ...
            for y in range(ny):
                for x in range(nx):
                    expl += f' · pixel at y ={y:3d}, x ={x:3d}:  ({safeprint(safeget(data,y,x,0)):>11s}, {safeprint(safeget(data,y,x,1)):>11s}, {safeprint(safeget(data,y,x,2)):>11s})\n'
            expl += '\n'
    if oe.get('wrong_output'):
        ...
        for k in ['y0', 'x0', 'y1', 'x1']:
            ...
            if a != b:
                expl += f'{k:>15s} {hl}{safeprint(a):>15s} {safeprint(b):>15s}{reset}\n'
            else:
                expl += f'{k:>15s} {safeprint(a):>15s} {safeprint(b):>15s}\n'
        for k in ['inner', 'outer']:
            for i in [0, 1, 2]:
                a = safeget(output, k, i)
                b = safeget(expected, k, i)
                ...
                if abs(safenum(a) - safenum(b)) > threshold > 0:
                    expl += f'{label:>15s} {hl}{safeprint(a):>15s} {safeprint(b):>15s}{reset}\n'
                else:
                    expl += f'{label:>15s} {safeprint(a):>15s} {safeprint(b):>15s}\n'
